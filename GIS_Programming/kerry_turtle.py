...
def convert_point(ratios, point):
    """
    Function to convert easting/northing to screen coordinates. Remember screen coordinates are necessary for rendering.
...
def draw_data(data):
    ...
    for row in data:
        ...
        t.goto(convert_point(ratios, centroid))
        ...
        for geom in coords.geoms:
            for point in list(geom.exterior.coords):
                t.goto(convert_point(ratios, point))
                t.down()
            t.up()
